...
export default function Card({image: image, ProjectName: ProjectName, ProjectDescription: ProjectDescription, LiveUrl: LiveUrl, githubUrl: githubUrl, TechStack: TechStack}: {
    ...
    return (
        <div className="bg-(--bg-surface) min-h-125 w-full border border-transparent rounded-2xl shadow-card transition duration-300 hover:border-[var(--border-hover)] hover:bg-[var(--bg-card)] hover:shadow-[0_0_40px_rgba(112,105,255,0.4)] flex flex-col">
            ...
            <div className="p-8 flex flex-col gap-4">
                <h4>{ProjectName}</h4>
                <p className="text-(--text-muted)">{ProjectDescription}</p>
                ...
                <div className="flex gap-5">
                    <div>
                        <Link href={LiveUrl} target="_blank" className="flex gap-2 items-center">
                            <SquareArrowOutUpRightIcon className="w-4 h-4 text-(--text-muted)" />
                            <span className="text-(--text-muted)">Live Demo</span>
                        </Link>
                    </div>
                    <div>
